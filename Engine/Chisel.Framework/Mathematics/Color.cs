...
public struct Color : IEquatable<Color>, IFormattable
{
    public byte R
    {
        get => unchecked((byte)_value);
        set => _value = (_value & 0xffffff00) | (value);
    }

    public byte G
    {
        get => unchecked((byte)(_value >> 8));
        set => _value = (_value & 0xffff00ff) | ((uint)value << 8);
    }

    public byte B
    {
        get => unchecked((byte)(_value >> 16));
        set => _value = (_value & 0xff00ffff) | ((uint)value << 16);
    }

    public byte A
    {
        get => unchecked((byte)(_value >> 24));
        set => _value = (_value & 0x00ffffff) | ((uint)value << 24);
    }

    // Stole these from MonoGame
    ...
    // Packed RGBA value, stored ABGR (because little endian)
    // This basically just a hex code
    private uint _value;
